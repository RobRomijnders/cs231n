...
#$x_t\in\mathbb{R}^D$ and the previous hidden state $h_{t-1}\in\mathbb{R}^H$; 
#the LSTM also maintains an $H$-dimensional *cell state*, so we also receive the 
#previous cell state $c_{t-1}\in\mathbb{R}^H$. The learnable parameters of the LSTM 
#are an *input-to-hidden* matrix $W_x\in\mathbb{R}^{4H\times D}$, a *hidden-to-hidden* 
#matrix $W_h\in\mathbb{R}^{4H\times H}$ and a *bias vector* $b\in\mathbb{R}^{4H}$.
## 
