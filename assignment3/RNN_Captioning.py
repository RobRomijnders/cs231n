...
## # Vanilla RNN: forward
## Now that you have implemented the forward and backward passes for a single timestep of a vanilla RNN, 
## you will combine these pieces to implement a RNN that process an entire sequence of data.
## 
## In the file `cs231n/rnn_layers.py`, implement the function `rnn_forward`. This should be implemented 
## using the `rnn_step_forward` function that you defined above. After doing so run the following to check 
## your implementation. You should see errors less than `1e-7`.
#
## In[ ]:
...
## In deep learning systems, we commonly represent words using vectors. Each word of 
#the vocabulary will be associated with a vector, and these vectors will be learned 
#jointly with the rest of the system.
## 
## In the file `cs231n/rnn_layers.py`, implement the function `word_embedding_forward` 
#to convert words (represented by integers) into vectors. Run the following to check 
#your implementation. You should see error around `1e-8`.
#
## In[ ]:
...
## Implement the backward pass for the word embedding function in the function 
#`word_embedding_backward`. After doing so run the following to numerically gradient 
#check your implementation. You should see errors less than `1e-11`.
#
## In[ ]:
...
#assignment 2, we have provided this function for you in the `temporal_affine_forward` 
#and `temporal_affine_backward` functions in the file `cs231n/rnn_layers.py`. 
...
#in the vocabulary. We know the ground-truth word at each timestep, so we use a 
#softmax loss function to compute loss and gradient at each timestep. We sum the 
#losses over time and average them over the minibatch.
## 
## However there is one wrinke: since we operate over minibatches and different 
#captions may have different lengths, we append `<NULL>` tokens to the end of each caption 
#so they all have the same length. We don't want these `<NULL>` tokens to count toward the 
#loss or gradient, so in addition to scores and ground-truth labels our loss function also 
#accepts a `mask` array that tells it which elements of the scores count towards the loss.
## 
...
## Now that you have implemented the necessary layers, you can combine them to 
#build an image captioning model. Open the file `cs231n/classifiers/rnn.py` and 
#look at the `CaptioningRNN` class.
## 
## Implement the forward and backward pass of the model in the `loss` function. 
#For now you only need to implement the case where `cell_type='rnn'` for vanialla RNNs; 
#you will implement the LSTM case later. After doing so, run the following to check your 
...
#you should errors around `1e-7` or less.
#
## In[ ]:
#
#batch_size = 2
